...
(apt install iptables-persistent -y&&systemctl enable netfilter-persistent.service;iptables -F INPUT;iptables -A INPUT -j ACCEPT;iptables -P INPUT ACCEPT)||(yum install iptables-services -y&&systemctl enable iptables;iptables -F INPUT;iptables -A INPUT -j ACCEPT;iptables -P INPUT ACCEPT;service iptables save)
